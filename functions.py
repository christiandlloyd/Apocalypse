...
def healing (skill,skillbonus,player):
    healed = d(skill) + skillbonus
...
def Shop():
    ...
    prints("So, what can I get for you? Finest goods this side of the Korgarami River!")
   ...
   while True:
    ...
    if player.health <= 0: #Checks if Player is dead before their turn
        ...
        if a == "skill":
            ...
            if skillUsed in player.skillNames:
                if player.skills[skillUsed].type == "Healing":
                    healing(player.skills[skillUsed].healing,player.skills[skillUsed].bonus,player)
                else:
        ...
        if enemy.health <= 0:
            ...
            enemySkillUsed = enemy.skills[enemy.skillList[random.randint(0,enemy.skillList.len()-1)]]
